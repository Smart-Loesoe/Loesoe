...
export default function Dashboard() {
    ...
    const init = async () => {
      ...
      if (!token) {
        try {
          ...
          setError(
            `Netwerkfout bij automatisch inloggen: ${
              e?.message ?? String(e)
            }`
          );
          setLoading(false);
  ...
  const handleSendChat = async () => {
    ...
    if (!token) {
      setChatError("Geen token gevonden. Log opnieuw in via /login.");
    ...
    // 1️⃣ SSE-stream openen naar /stream/chat
    const url = `${API_BASE}/stream/chat?q=${encodeURIComponent(trimmed)}`;
    ...
    es.onerror = () => {
      setIsStreaming(false);
      setChatError("Streamfout of verbinding verbroken.");
      es.close();
      streamRef.current = null;
    };
    ...
    // 2️⃣ Parallel: zelflerend endpoint /chat/learning aanroepen
    try {
      const res = await fetch(`${API_BASE}/chat/learning`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
      ...
      if (!res.ok) {
        const txt = await res.text().catch(() => "");
        setChatError(
          `Fout bij /chat/learning (status ${res.status}).\n\n${
            txt || "Geen extra info."
          }`
        );
      } else {
        ...
        if (json.analysis) {
          ...
          const mapped: ChatAnalysis = {
            ...
            emotion: typeof a.emotion === "string" ? a.emotion : null,
      ...
      setChatError(
        `Netwerkfout bij /chat/learning: ${e?.message ?? String(e)}`
      );
    } finally {
      setChatLoading(false);
    }
      ...
      >
        ...
        >
          <div>
            ...
            >
              Elke boodschap wordt geanalyseerd én Loesoe streamt het antwoord
              realtime.
            </p>
          </div>
          {flashLearning && (
